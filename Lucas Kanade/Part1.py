...
def Lucas_Kanade(image1, image2, points, w):
    oldframe = image1
    ...
    u = np.ones(I1.shape)
    v = np.ones(I1.shape)
    status=np.zeros(points.shape[0]) # this will tell change in x,y
    #A = np.zeros((2, 2))
    #B = np.zeros((2, 1))
...
def track(frames):
    ...
    for i in range(1, len(frames)):
        ...
        """
        frame1 = frames[i]
        temp, track_points = Lucas_Kanade(frame0, frame1, track_points,2)
...
biped_vid = mpy.VideoFileClip(str(d) + "/HW4/biped_1.avi")
...
new_frames = track(frames)
    

clip = mp.ImageSequenceClip(new_frames, fps=video_fps)
clip.write_videofile(str(d) + "/HW4/walker.mp4", codec='libx264')
